...
pub static VERTICES: Mutex<Vec<Vertex>> = Mutex::new(Vec::new());
/*const VERTICES: &[Vertex] = &[
    Vertex { position: [-0.0868241, 0.49240386, -0.5], tex_coords: [0.4131759, 0.00759614], }, // 0
...
pub static INDICES: Mutex<Vec<u16>> = Mutex::new(Vec::new());
/*const INDICES: &[u16] = &[
    0, 1, 4,
    1, 2, 4,
    2, 3, 4,
...
impl<'a> RenderState<'a> {
    pub async fn new(window: &'a Window) -> Self {
        ...
        let camera_controller = crate::camera::CameraController::new(0.2);
        ...
        let num_indices = INDICES.lock().unwrap();
        let index_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Index Buffer"),
            contents: bytemuck::cast_slice(&num_indices),
            usage: wgpu::BufferUsages::INDEX,
        });
        let num_indices = num_indices.len() as u32;
    ...
    pub fn cam_dir(&mut self, e: Euler<Deg<f32>>) {
    ...
    pub fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        self.idx_num = num_indices;

        std::mem::drop(vertices);
